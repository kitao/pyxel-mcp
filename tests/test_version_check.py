...
from pyxel_mcp._common.pyxel_env import check_updates
...
def test_network_failure_returns_empty():
    with patch("pyxel_mcp._common.pyxel_env.urlopen", side_effect=Exception("offline")):
        lines = check_updates()
    assert lines == []
...
def test_partial_failure():
    ...
    with patch("pyxel_mcp._common.pyxel_env.urlopen", flaky_urlopen), \
         patch("pyxel_mcp._common.pyxel_env.installed_version", return_value="1.0.0"):
        lines = check_updates()
