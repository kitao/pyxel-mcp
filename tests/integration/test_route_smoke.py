...
from pyxel_mcp.server import read_palette as read_palette_tool
from pyxel_mcp.server import run as run_tool
from pyxel_mcp.server import validate as validate_tool
...
def test_validate_route_clean():
    result = structured(validate_tool(script=ROUTE))
...
def test_route_palette_is_observable():
    result = structured(read_palette_tool(script=ROUTE))
