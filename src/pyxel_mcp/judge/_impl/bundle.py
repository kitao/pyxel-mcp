...
enough captured frames, the audio files declared in the manifest, and that
the captured frames actually move (mid-bundle pixel diff > threshold).
"""
...
def _dead_time_diff(frames_dir: Path) -> tuple[float, dict[str, Any]]:
    ...
    return float(ratio), {"first": str(first), "mid": str(mid), "ratio": ratio}
